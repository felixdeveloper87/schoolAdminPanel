...
export default async function AlunoPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="relative overflow-hidden rounded-[26px] border border-border bg-gradient-to-br from-white via-white to-brand/10 p-5 shadow-[0_16px_45px_rgba(37,49,79,.08)] sm:p-6">
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-center">
            <div className="w-fit rounded-[26px] bg-card p-1.5 shadow-[0_10px_28px_rgba(52,63,94,.12)]">
              <StudentAvatar photoUrl={student.photoUrl} name={student.fullName} size="lg" />
            </div>
            <div className="min-w-0">
              <p className="text-[11px] font-extrabold uppercase tracking-[0.12em] text-brand">Ficha do aluno</p>
              <h1 className="mt-1 truncate font-display text-3xl font-extrabold tracking-tight text-foreground">{student.fullName}</h1>
              <p className="mt-1.5 text-sm text-muted-foreground">
                {formatAge(student.birthDate)} <span className="px-1 text-muted-foreground">•</span> Nascimento em {formatDate(student.birthDate)}
              </p>
          ...
          <div className="flex flex-wrap items-center gap-2 lg:max-w-[430px] lg:justify-end">
            <Link
              ...
              className={buttonVariants({ variant: 'outline', size: 'sm', className: 'h-9 rounded-xl border-border bg-card' })}
            >
      ...
      {student.status === 'INACTIVE' && (
        <div className="flex gap-3 rounded-[18px] border border-border bg-muted/60 p-4">
          <span className="grid h-9 w-9 flex-none place-items-center rounded-xl bg-card text-muted-foreground shadow-sm"><UserRound className="h-4 w-4" /></span>
          <div>
            <p className="text-sm font-extrabold text-foreground">Ex-aluno desde {student.inactiveAt ? formatDate(student.inactiveAt) : 'data não informada'}</p>
            <p className="mt-0.5 text-xs text-muted-foreground">Motivo: {student.inactiveReason ?? 'Não informado'}</p>
          </div>
        </div>
      )}
      ...
      <div className="grid gap-5 xl:grid-cols-[1.35fr_.85fr]">
            ...
            <CardContent className="pt-5">
              {activeEnrollment ? (
                <dl className="grid gap-4 sm:grid-cols-2">
                  <div className="rounded-xl bg-muted/60 p-3"><dt className="text-[10px] font-bold uppercase text-muted-foreground">Turma</dt><dd className="mt-1 text-sm font-extrabold text-foreground">{activeEnrollment.classroom.name}</dd></div>
                  <div className="rounded-xl bg-muted/60 p-3"><dt className="text-[10px] font-bold uppercase text-muted-foreground">Faixa e turno</dt><dd className="mt-1 text-sm font-extrabold text-foreground">{AGE_GROUP_LABELS[activeEnrollment.classroom.ageGroup]} · {SHIFT_LABELS[activeEnrollment.classroom.shift]}</dd></div>
                  <div className="rounded-xl bg-muted/60 p-3"><dt className="text-[10px] font-bold uppercase text-muted-foreground">Início</dt><dd className="money mt-1 text-sm font-extrabold text-foreground">{formatDate(activeEnrollment.startDate)}</dd></div>
                  <div className="rounded-xl bg-muted/60 p-3">
                    <dt className="text-[10px] font-bold uppercase text-muted-foreground">Mensalidade</dt>
                    <dd className="money mt-1 text-sm font-extrabold text-foreground">{brl(effectiveMonthlyFee!)}</dd>
                    {activeEnrollment.discountCents > 0 && <p className="mt-1 text-[10px] text-muted-foreground">Original {brl(activeEnrollment.monthlyFeeCents)} · desconto {DISCOUNT_REASON_LABELS[activeEnrollment.discountReason]}</p>}
                  </div>
                </dl>
              ) : (
                <div className="py-8 text-center">
                  <span className="mx-auto grid h-11 w-11 place-items-center rounded-2xl bg-muted/60 text-muted-foreground"><School className="h-5 w-5" /></span>
                  <p className="mt-3 font-bold text-foreground">Sem matrícula ativa</p>
                  <p className="mt-1 text-xs text-muted-foreground">{financial ? `Responsável financeiro: ${financial.fullName}.` : 'Cadastre um responsável financeiro antes de matricular.'}</p>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <div className="space-y-5">
          {hasHealthNotes && (
            <Card className="overflow-hidden rounded-[22px] border-destructive/20 bg-white/95 shadow-[0_12px_35px_rgba(74,45,48,.05)]">
              ...
              <CardContent className="space-y-3 pt-5 text-sm">
                {student.allergies && <div className="rounded-xl border border-destructive/20 bg-destructive/10 p-3"><p className="flex items-center gap-1.5 text-xs font-extrabold text-destructive"><ShieldAlert className="h-3.5 w-3.5" /> Alergias</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{student.allergies}</p></div>}
                {student.dietaryRestrictions && <div className="rounded-xl bg-accent/15 p-3"><p className="flex items-center gap-1.5 text-xs font-extrabold text-accent-deep"><Utensils className="h-3.5 w-3.5" /> Restrições alimentares</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{student.dietaryRestrictions}</p></div>}
                {student.medicalNotes && <div className="rounded-xl bg-muted/60 p-3"><p className="text-xs font-extrabold text-foreground">Observações médicas</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{student.medicalNotes}</p></div>}
              </CardContent>
            </Card>
          )}
          ...
          <Card className={panelClassName}>
            <CardHeader className="flex-row items-center gap-3 space-y-0 border-b border-border">
              <span className="grid h-9 w-9 place-items-center rounded-xl bg-brand/10 text-brand"><UserRound className="h-4 w-4" /></span>
              <div><CardTitle className="text-lg text-foreground">Responsáveis</CardTitle><p className="text-[11px] text-muted-foreground">Contatos e autorizações</p></div>
            </CardHeader>
            <CardContent className="space-y-3 pt-5">
              {student.guardians.map((guardian) => (
                <div key={guardian.id} className="rounded-[16px] border border-border bg-muted/60 p-3.5 text-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="truncate font-extrabold text-foreground">{guardian.fullName}</p>
                      <p className="mt-0.5 text-[11px] text-muted-foreground">{RELATIONSHIP_LABELS[guardian.relationship]}</p>
                    </div>
                    <a
                      href={`https://wa.me/55${guardian.phoneWhatsapp}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="grid h-9 w-9 flex-none place-items-center rounded-xl bg-success/10 text-success hover:bg-success/10"
                      aria-label={`Abrir WhatsApp de ${guardian.fullName}`}
                    >
                      <MessageCircle className="h-4 w-4" />
                    </a>
                  </div>
                  <p className="money mt-2 text-xs text-muted-foreground">{guardian.phoneWhatsapp}</p>
                  {guardian.email && <a href={`mailto:${guardian.email}`} className="mt-1 flex items-center gap-1.5 truncate text-[11px] text-muted-foreground hover:text-brand"><Mail className="h-3 w-3" /> {guardian.email}</a>}
          ...
          {student.notes && (
            <Card className={panelClassName}>
              <CardHeader className="flex-row items-center gap-3 space-y-0 border-b border-border">
                <span className="grid h-9 w-9 place-items-center rounded-xl bg-accent/15 text-accent-deep"><NotebookText className="h-4 w-4" /></span>
                <CardTitle className="text-lg text-foreground">Observações</CardTitle>
              </CardHeader>
              <CardContent className="pt-5 text-sm leading-relaxed text-muted-foreground">{student.notes}</CardContent>
            </Card>
          )}
        </div>
      </div>
