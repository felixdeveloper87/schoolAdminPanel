    ...
    <div className="space-y-7">
      <section className="relative overflow-hidden rounded-[28px] bg-[#1d4962] px-5 py-6 text-white shadow-[0_18px_45px_rgba(29,73,98,.2)] sm:px-7 sm:py-7">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-5">
          ...
          <WaitlistEntryDialog
            trigger={
              <Button className="h-10 rounded-xl bg-white px-4 text-[#1c506b] shadow-[0_8px_20px_rgba(0,0,0,.16)] hover:bg-[#edfbff] hover:text-[#1c506b]">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
        <Card className="overflow-hidden rounded-[24px] border-white bg-white/95 shadow-[0_1px_2px_rgba(16,24,40,.03),0_14px_35px_rgba(34,45,75,.08)]">
          <CardContent className="flex flex-wrap items-center justify-between gap-4 p-5">
            <div className="flex items-center gap-3">
              <span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#eaf7fb] text-[#3e8eb2]">
                <CalendarClock className="h-5 w-5" />
              </span>
              <div>
                <p className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#71889b]">Prioridade de contato</p>
                <h2 className="mt-0.5 font-display text-lg font-extrabold text-[#20394f]">
                  {priorityEntry ? priorityEntry.childName : 'Nenhum contato pendente'}
                </h2>
                <p className="mt-0.5 text-xs text-[#748398]">
                  {priorityEntry
        ...
        <Card className="overflow-hidden rounded-[24px] border-[#e5e7f2] bg-gradient-to-br from-[#fbfcff] to-[#f1f4ff] shadow-[0_1px_2px_rgba(16,24,40,.03),0_14px_35px_rgba(34,45,75,.08)]">
          <CardContent className="flex h-full items-center gap-3 p-5">
            <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#e8edff] text-[#5e6ed1]">
              <UserPlus className="h-5 w-5" />
            </span>
            <p className="text-sm leading-relaxed text-[#66718c]">
              <strong className="text-[#3f4c6b]">Ideia:</strong> priorize contatos que estão aguardando há mais tempo antes de abrir novas vagas.
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <Link
          ...
          className={cn(
            ...
            !status
              ...
              : 'border-white bg-white/85 text-[#52677c] shadow-sm hover:-translate-y-0.5 hover:border-[#adc1d2] hover:text-[#285d7b]',
          )}
        >
      ...
      <Card className="overflow-hidden rounded-[24px] border-white bg-white/95 shadow-[0_1px_2px_rgba(16,24,40,.03),0_14px_35px_rgba(34,45,75,.08)]">
        <div className="flex items-center justify-between border-b border-[#e7edf5] px-5 py-4">
          <div>
            <h2 className="font-display text-lg font-extrabold text-[#20394f]">Entradas da lista</h2>
            <p className="mt-0.5 text-xs text-[#718197]">{visibleEntries.length} registro(s) exibido(s).</p>
          </div>
        </div>
        <Table>
          <TableHeader className="bg-[#f5f9fc]">
            <TableRow>
            ...
            {visibleEntries.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="py-14 text-center text-muted-foreground">
                  <span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-[#eff5f8] text-[#7690a0]">
                    <UserRoundSearch className="h-5 w-5" />
                  </span>
                  <p className="mt-3 font-bold text-[#385067]">Nenhuma entrada encontrada</p>
                  <p className="mt-1 text-xs text-[#7d8ca0]">Ajuste o filtro ou registre uma nova entrada.</p>
                </TableCell>
              </TableRow>
            )}
            {visibleEntries.map((entry) => (
              <TableRow key={entry.id} className="hover:bg-[#f5faff]">
                <TableCell>
                  <div className="flex items-center gap-3">
                    <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#edf6fa] text-sm font-extrabold text-[#3c7b9b]">
                      {entry.childName.split(' ').filter(Boolean).slice(0, 2).map((part) => part[0]).join('').toUpperCase()}
                    </span>
                    <div className="min-w-0">
                      <p className="truncate font-bold text-[#294259]">{entry.childName}</p>
                      <p className="text-xs text-[#738197]">{formatAge(entry.birthDate)}</p>
                      {entry.notes && <p className="mt-1 line-clamp-1 text-[11px] text-[#8794a4]">{entry.notes}</p>}
                    </div>
                  </div>
                </TableCell>
                <TableCell className="hidden sm:table-cell">
                  <p className="font-semibold text-[#5b6a81]">{AGE_GROUP_LABELS[entry.desiredAgeGroup]}</p>
                  <p className="text-xs text-[#8490a2]">{SHIFT_LABELS[entry.desiredShift]}</p>
                </TableCell>
                <TableCell className="hidden md:table-cell">
                  <p className="font-semibold text-[#59697f]">{entry.guardianName}</p>
                  <a
                    href={`https://wa.me/55${entry.phoneWhatsapp}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="money text-xs font-semibold text-[#2d9c75] hover:underline"
                  >
                    {entry.phoneWhatsapp}
                  </a>
                </TableCell>
                <TableCell className="money hidden text-[#66768d] lg:table-cell">{formatDate(entry.requestedAt)}</TableCell>
                <TableCell>
